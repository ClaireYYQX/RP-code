import pandas as pd
from MLModelRunner import MLModelRunner
...
class Runner:
    ...
    def run(self):
        ...
        mean_mae_df.to_csv(f'result_csv/mae_mean_ROI_{str(self.random_seed_list[0])}-{str(self.random_seed_list[-1])}.csv', index=True)
        mean_r2_df.to_csv(f'result_csv/r2_mean_ROI_{str(self.random_seed_list[0])}-{str(self.random_seed_list[-1])}.csv', index=True)
    ...
    def performance_boxplot(self, scores_df, metric, random_seed):
        ...
        plt.ylabel(metric)
        plt.show()     
        directory = '/imaging/correia/wl05/final_scripts/ROI/performance_plots/'  
        filename = metric+str(random_seed)
    ...
    def average_boxplot(self, mean_df, metric):
        plt.figure(figsize=(10, 6))
        sns.boxplot(data=mean_df, showfliers=False)
        plt.title('Comparison for ' + metric)
        plt.ylabel(metric)
        plt.show()     
        directory = '/imaging/correia/wl05/final_scripts/ROI/performance_plots/'  
        filename = metric+'_'+str(self.random_seed_list[0])+'-'+str(self.random_seed_list[-1])
        plt.savefig(f'{directory}{filename}')
