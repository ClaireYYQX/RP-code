...
def get_feature_matrix(): 
    ...
    img_md = nib.load(os.path.join(folder_path, 'all_fw_MS_MD.nii.gz')).get_fdata()
    # img_md = nib.load(os.path.join(folder_path, 'all_fw_MS_MD_skeletonised.nii.gz')).get_fdata()
    flattened_md = np.transpose(img_md.flatten().reshape(-1, 620))
    print(flattened_md.shape)
    feature_names = [str(i)+'_'+str(j)+'_'+str(k) for i in range(182) for j in range(218) for k in range(182)]
    df_md = pd.DataFrame(flattened_md, columns = feature_names)
    df_md = df_md.loc[:, df_md.any()]
    df_md = remove_outliers(df_md)
    print(df_md.shape)
    return df_md
...
def save_folds_to_file(modality, df, train_test, fold, random_seed, k):
    directory = '/imaging/correia/wl05/final_scripts/Voxel/train_test_data/FWE/'
...
df_md = get_feature_matrix()
Age = get_participants_age()
modality = 'MD'

for random_seed in range(41, 51):
    print(random_seed)
    cv_stratify_split(modality, df_md, Age, random_seed)