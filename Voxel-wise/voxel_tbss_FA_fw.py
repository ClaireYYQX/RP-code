...
def get_feature_matrix(): 
    ...
    img_fa = nib.load(os.path.join(folder_path, 'all_fw_MS_FA.nii.gz')).get_fdata()
    # img_fa = nib.load(os.path.join(folder_path, 'all_fw_MS_FA_skeletonised.nii.gz')).get_fdata()
...
def get_subsample_index(rs=30):
    subsample_file = pd.DataFrame(pd.read_csv('subsample/subsample_index_even_30.csv'))
    print(subsample_file)
    subsample_index = subsample_file['Unnamed: 0'].tolist()
