...
def get_feature_matrix(): 
    ...
    img_fw = nib.load(os.path.join(folder_path, 'all_fw_MS_FW.nii.gz')).get_fdata()
    # img_fw = nib.load(os.path.join(folder_path, 'all_fw_MS_FW_skeletonised.nii.gz')).get_fdata()
    flattened_fw = np.transpose(img_fw.flatten().reshape(-1, 620))
    print(flattened_fw.shape)
    feature_names = [str(i)+'_'+str(j)+'_'+str(k) for i in range(182) for j in range(218) for k in range(182)]
    df_fw = pd.DataFrame(flattened_fw, columns = feature_names)
    df_fw = df_fw.loc[:, df_fw.any()]
    df_fw = remove_outliers(df_fw)
    print(df_fw.shape)
    return df_fw
